...
We merged records from the sim-com database to records from the PPMF, by exact linkage on census block and voting-age status (i.e. $\mathbf{1}_{\text{age} \geq 18}$) for individuals in the PPMF residing in households, and counted the number of individuals in the sim-com database who were (1) linked to a unique individual in the PPMF; and (2) linked to one or more individuals with a unique race or ethnicity value.  We also calculated the number and fraction of individuals for whom a unique linked race/ethnicity attribute matched the value in the test database.
